...
public class CommonRecogParams {
     ...
     *字符串格式的参数
     */
    private ArrayList<String> stringParams =new ArrayList<>();

    /**
     *int 格式的参数
     */
    private ArrayList<String> intParams = new ArrayList<>();
    /**
     *bool格式的参数
     */
    private ArrayList<String> boolParams = new ArrayList<>();
    ...
    public CommonRecogParams (Activity context){
        ...
        intParams.addAll(Collections.singleton(SpeechConstant.PROP));

        boolParams.addAll(Collections.singleton(SpeechConstant.DISABLE_PUNCTUATION));
    ...
    protected void initSamplePath(Activity context) {
        ...
        Log.i(TAG,"samplePath1: "+samplePath);
        if (!makeDir(samplePath)) {
            samplePath = context.getApplication().getExternalFilesDir(sampleDir).getAbsolutePath();
            Log.i(TAG,"afterPath: "+samplePath);
            if (!makeDir(samplePath)) {
